...
class TestDCOPFAgent(BaseTest):
    ...
    def test_ANM6Easy_horizon_1(self):
        ...
        agent = MPCAgentConstant(self.env.simulator, self.env.action_space, self.safety_margin, planning_steps=1)
    ...
    def test_ANM6Easy_horizon_3(self):
        ...
        agent = MPCAgentConstant(self.env.simulator, self.env.action_space, self.safety_margin, planning_steps=3)
    ...
    def test_ANM6Easy_horizon_20(self):
        ...
        agent = MPCAgentConstant(self.env.simulator, self.env.action_space, self.safety_margin, planning_steps=20)
    ...
    def _check_ANM6Easy_constraints(self, agent):
        ...
        P = [
            self.B[0, 1] * (V_ang[0] - V_ang[1]),
            self.B[1, 0] * (V_ang[1] - V_ang[0])
            + self.B[1, 2] * (V_ang[1] - V_ang[2])
            + self.B[1, 3] * (V_ang[1] - V_ang[3]),
            self.B[2, 1] * (V_ang[2] - V_ang[1])
            + self.B[2, 4] * (V_ang[2] - V_ang[4])
            + self.B[2, 5] * (V_ang[2] - V_ang[5]),
        ...
        for i in [1, 3, 5]:
            npt.assert_allclose(self.env.simulator.devices[i].p, agent.P_dev.value[i], atol=self.tolerance)

        # C3) P_gen and P_des are in [p_min, p_max].
        for i in [2, 4, 6]:
            self.assert_almost_less_than(self.env.simulator.devices[i].p_min, agent.P_dev.value[i])
            self.assert_almost_less_than(agent.P_dev.value[i], self.env.simulator.devices[i].p_max)

        # C4) P_gen <= P_max for each generator.
        for i in [2, 4]:
            self.assert_almost_less_than(agent.P_dev.value[i], self.env.simulator.devices[i].p_pot)

        # C5) New soc in [soc_min, soc_max].
        new_soc = self.env.simulator.devices[6].soc - agent.P_dev.value[6] * self.env.delta_t
        self.assert_almost_less_than(self.env.simulator.devices[6].soc_min, new_soc)
        self.assert_almost_less_than(new_soc, self.env.simulator.devices[6].soc_max)
        ...
        self.assertAlmostEqual(0.0, V_ang[0])
