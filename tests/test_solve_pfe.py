...
from gym_anm.simulator.solve_pfe import solve


class TesSolvePFE(unittest.TestCase):
    def setUp(self):
        pass
    ...
    def test_2bus(self):
        ...
        Q = np.array([0, 1])
        V = solve(Y, P, Q, 0, 1.)
        s1 = V[1] * np.conj(Y[1, 0] * V[0] + Y[1, 1] * V[1])
        npt.assert_allclose(s1.real, P[1])
        npt.assert_allclose(s1.imag, Q[1])
